...
from formata.core.ir import CellAddress, CellRange, SpreadsheetBlockKind
...
def plan_document(document: SpreadsheetDocument) -> DocumentPlan:
    ...
    """
    return DocumentPlan(
        worksheets=tuple(plan_worksheet(sheet) for sheet in document.worksheets),
    )


def plan_worksheet(worksheet: Worksheet) -> WorksheetPlan:
...
def _place_block(
    ...
    *,
    ...
    rows, columns = _measure(block, anchor=anchor, path=path, placements=nested)
