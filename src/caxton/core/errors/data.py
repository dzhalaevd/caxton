...
class DataSourceError(CaxtonError):
...
@dataclasses.dataclass(eq=False)
class AggregateEvaluationError(DataEvaluationError):
    """Raised when an aggregation callable or its result is invalid."""


@dataclasses.dataclass(eq=False)
class GroupingError(DataEvaluationError):
    """Raised when declared group values cannot be ordered."""


@dataclasses.dataclass(eq=False)
class MatrixConflictError(DataEvaluationError):
