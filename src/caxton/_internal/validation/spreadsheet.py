...
from caxton.core.models.expressions import _contains_aggregate
...
def _validate_spreadsheet_features(  # noqa: C901, WPS213
    document: SpreadsheetDocument,
    notification: Notification,
) -> None:
    for worksheet in document.worksheets:
        ...
        for index, block in enumerate(iter_blocks(worksheet.blocks)):
            if isinstance(block, Matrix):
                _validate_matrix(
                    block,
                    document,
                    f'worksheet["{worksheet.name}"].matrix[{index}]',
                    notification,
                )


def _validate_grouping_and_aggregation(  # noqa: C901
    ...
    nested = tuple(
        ...
        and _contains_aggregate(column.source)
    )
...
def _validate_matrix(
    ...
    for column in columns:
        ...
        if not isinstance(column.source, Expression):
            continue
        for reference in _expression_references(column.source):
            notification.add(
...
def _aggregate_references(expression: AggregateExpr) -> Iterator[str]:
    ...
    if expression.where is not None:
        yield from _expression_references(expression.where)
